...
for aa in combined_df["aa_type"].unique():
    ...
    for dna in ["DA", "DT", "DG", "DC"]:
        dna_df = aa_df[aa_df["dna_type"] == dna]
        dna_dir = os.path.join(aa_dir, dna)
        ...
        for inter in ["bb_to_P", "bb_to_S", "bb_to_B", "sc_to_P", "sc_to_S", "sc_to_B"]:
            ...
            if inter_df.empty:
                continue
            inter_dir = os.path.join(dna_dir, inter)
            ...
            plot_log_heatmap(r_flat, theta_flat, u_flat,
                             "r (Å)", "θ (°)",
                             f"U(r, θ) | {aa} to {dna} via {inter}",
                             os.path.join(inter_dir, f"{aa}_{dna}_{inter}_heatmap_r_theta.png"))

            plot_log_heatmap(r_flat, phi_flat, u_flat,
                             "r (Å)", "φ (°)",
                             f"U(r, φ) | {aa} to {dna} via {inter}",
                             os.path.join(inter_dir, f"{aa}_{dna}_{inter}_heatmap_r_phi.png"))

            plot_log_heatmap(theta_flat, phi_flat, u_flat,
                             "θ (°)", "φ (°)",
                             f"U(θ, φ) | {aa} to {dna} via {inter}",
                             os.path.join(inter_dir, f"{aa}_{dna}_{inter}_heatmap_theta_phi.png"))

            # Save 3D plot with title
            plot_3d_potential(r_flat, theta_flat, phi_flat, u_flat,
                              os.path.join(inter_dir, f"{aa}_{dna}_{inter}_3D_U_total_plot.png"))
